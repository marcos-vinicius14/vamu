...
const { schema, state, loading, onSubmit } = useCreateEvent()
</script>
  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950 py-12 px-4 sm:px-6">
    <div class="max-w-2xl mx-auto">
      <UForm :schema="schema" :state="state" @submit="onSubmit"
        class="space-y-8 bg-white dark:bg-gray-900 p-8 rounded-xl shadow-sm ring-1 ring-gray-200 dark:ring-gray-800">

        <div class="space-y-2 border-b border-gray-200 dark:border-gray-800 pb-4">
          <UInput v-model="state.title" variant="none" placeholder="Nome do Evento" size="xl" class="p-0" :ui="{
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 dark:bg-gray-800/50 p-6 rounded-lg">
          <UFormField label="Data" name="date" class="w-full"
            :ui="{ label: 'text-xs font-medium text-gray-500 uppercase tracking-wider mb-1' }">
            <UInput v-model="state.date" type="datetime-local" icon="i-heroicons-calendar" variant="outline"
              color="neutral" class="w-full" />
          </UFormField>

          <UFormField label="Local" name="location" class="w-full"
            :ui="{ label: 'text-xs font-medium text-gray-500 uppercase tracking-wider mb-1' }">
            <UInput v-model="state.location" placeholder="Endereço ou Link" icon="i-heroicons-map-pin" variant="outline"
              color="neutral" class="w-full" />
          </UFormField>
        </div>
        ...
        <div class="space-y-4">
          ...
          <UFormField name="description" class="w-full">
            <UTextarea v-model="state.description" variant="none"
              placeholder="Digite os detalhes do evento aqui... O que vai acontecer?" :rows="6" autoresize size="lg"
              class="w-full" :ui="{
        ...
        <div class="pt-6 border-t border-gray-100 dark:border-gray-800 flex justify-end">
          <UButton type="submit" size="xl" color="neutral" variant="solid" :loading="loading"
            icon="i-heroicons-arrow-right" trailing class="w-full md:w-auto px-8"

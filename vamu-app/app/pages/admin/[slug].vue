            ...
            <div v-else-if="dashboard" class="space-y-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ dashboard.event.title }}
                                </h1>
                ...
                <UCard>
                    <template #header>
                        ...
                        <p class="text-sm text-gray-500">Compartilhe o link abaixo para seus convidados confirmarem
                            presença.</p>
                    </template>

                    <div class="flex flex-col sm:flex-row gap-3">
                        <UButton icon="i-simple-icons-whatsapp" color="primary" variant="solid"
                            label="Enviar no WhatsApp" block @click="shareOnWhatsApp" />
                        <UButton icon="i-heroicons-clipboard-document" color="neutral" variant="solid"
                            label="Copiar Link" block @click="copyLink" />
                    </div>
                </UCard>
                    ...
                    <UCard>
                        <div class="text-center">
                            ...
                            <dd class="mt-1 text-3xl font-semibold text-gray-900 dark:text-white">{{
                                dashboard.stats.totalGuests }}</dd>
                        </div>
                    </UCard>
                    <UCard>
                        <div class="text-center">
                            <dt class="text-sm font-medium text-green-500">Confirmados</dt>
                            <dd class="mt-1 text-3xl font-semibold text-green-600">{{ dashboard.stats.confirmedCount }}
                            </dd>
                        </div>
                    </UCard>
                    <UCard>
                        <div class="text-center">
                            ...
                            <dd class="mt-1 text-3xl font-semibold text-red-600">{{ dashboard.stats.declinedCount }}
                            </dd>
                        </div>
                    </UCard>
                </div>
                ...
                <UCard :ui="{ body: 'p-0 sm:p-0' }">
                    <div class="p-4 border-b border-gray-100 dark:border-gray-800">
                        <UInput v-model="search" icon="i-heroicons-magnifying-glass" placeholder="Buscar convidados..."
                            color="info" :ui="{ trailing: 'pointer-events-auto' }">
                            <template #trailing>
                                <UKbd v-if="!search">K</UKbd>
                                <UButton v-else color="info" variant="link" icon="i-heroicons-x-mark" :padded="false"
                                    @click="clearSearch" />
                            </template>
                        </UInput>
                    </div>
